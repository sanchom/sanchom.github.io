...
On this panel, Michelle Davey presented the VPD's plans for a
drone/uncrewed aerial vehicle. They are still developing the policy
that will control its use, but she told us that it will primarily be
used for emergency response and for gathering data to analyze an
accident or crime that has already happened, especially vehicle
...
Josh Patterson expressed support for most uses of the drone that Davey
mentioned, with the exception of the potentially indiscriminate
...
I hope that that VPD's drone policy will disclose the factors that
they will use to determine whether a public event justifies the use of
a drone for pre-emptive monitoring and what is done with the data in
the case that no criminal investigations follow from the event.

The VPD also already operates a helicopter. While the use of a drone
may bring certain privacy issues to the forefront and may subject more
people more frequently to surveillance, it is not different in kind
from that which is already possible.

◊sub-heading{On predictive policing}
...
in the past. To avoid this, the system does not include property crime
that was reported by an officer; it only bases its predictions on
property crime that is reported by the property owner.
...
It is clear that CBSA requires no reasonable grounds (as would
normally be required for a search) to search your goods at the
border.◊note-cite["Customs Act" #:pinpoint "Section 99(1)(a)"] And
"goods, for greater certainty, includes conveyances, animals and any
document in any form."◊note-cite["Customs Act" #:pinpoint "Section
2(1)"]
...
Less clear is how the government is to treat the data that is on your
phone. Is it a 'good'? All we have today are some lower court
...
Edelmann noted that Canada doesn't have a consistent approach to data
that crosses the border. Data can cross the border over the internet
and data can cross the border on a cell phone. The data carried over
the border on a cell phone is minuscule compared to the data crossing
over the internet. But the government treats these two streams very
differently. He argued that the government needs to clarify with
legislation how ◊em{data} should be regulated and then do that
uniformly, predictably, and transparently, rather than shoehorning our
...
about their border-search practices, especially regarding the National
Targetting Program. What might increase the likelihood that your
